...
#for some reason I couldn't use raster without also having sp
library(sp)
library(raster)
library(ggplot2)
library(rgdal)
...
#don't need the scale argument when adding contrast stretch
...
falsecol <- stack(rdatB8, rdatB4, rdatB3)/10000
#false color image
plotRGB(falsecol, stretch="lin", maxpixels=rgbS@nrows*rgbS@ncols)
...
#reminder: 4=red, 3=green, 2=blue, 8=nearIR
...
rasterNDVI <- raster::extract(NDVI,landExtract[,2:3])
colnames(rasterNDVI) <- c("B02","B03","B04","B08")
#combine the original data with the coordinates with the raster data
rasterExNDVI <- cbind(landExtract,rasterNDVI)

ggplot(data=rasterExNDVI[rasterExNDVI$landcID==c("agri", "forest", "wetland"),], aes(x=landcID, y=rasterNDVI, fill=landcID))+
